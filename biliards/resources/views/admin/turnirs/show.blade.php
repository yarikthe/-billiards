...
<div class="d-flex justify-content-between">
   ...
   <div class="detail-info d-flex justify-content-between">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        @else
                            ...
                            <a href="{{ route('adminturnir.close',$show->id) }}" class="btn btn-light">Завершити</a>
                        @endif 
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        @if($show->isPiblic == 1)
                            ...
                            <a href="{{ route('adminturnir.hidden',$show->id) }}" class="btn btn-light">Скрить</a>
                        @else
                            <label class="btn btn-light"> Не опублікований</label>
                            <a href="{{ route('adminturnir.public',$show->id) }}" class="btn btn-primary">Опублікувати</a>
                        @endif 
                    </td>
                </tr>
                <tr>
                        ...
                        @foreach($players as $key => $value_id)
                            @foreach($player as $key => $value)
                                ...
                                @if($value_id->player_id == $value->id)
                                    <a href="{{ route('admin.show',$value->id) }}"> {{ $value->name }}</a>
        ...
        <a href="{{ route('adminturnir.table',$show->id) }}" class="btn btn-warning mb-2"><i class="fa fa-btn fa-trash fa-fw"></i> Таблиця раундів учасників</a> 
        ...
        @foreach($raund as $key => $value)
            <div class="card-raund-item bg-white shadow p-2 rounded-lg m-2">
                    ...
                    @else
                        ...
                        @endforeach
                    @endif
                    <a href="{{ route('adminturnir.delete-raund',$value->id) }}" class="btn btn-danger ">Видалити</a>
            </div>
         @endforeach
        </div>
    ...
    <hr>
   <a href="{{ route('adminturnir.delete',$show->id) }}" class="btn btn-danger">Видалити</a>
   <a href="{{ route('adminturnir.edit',$show->id) }}" class="btn btn-light">Редагувати</a>
