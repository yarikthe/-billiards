...
<div class="d-flex justify-content-between">
    ...
    <div class="action">
        <a href="/admin/turnir/create-new" class="btn btn-primary"><i class="fa fa-btn fa-trash fa-fw"></i> Створити</a> 
    </div>                        
   </div>
   <hr>
            ...
            @foreach($turnirsclose as $key => $value)
                <div class="card-turnir shadow p-2 bg-light rounded-lg text-dark">
                    <div class="header d-flex justify-content-between mb-2">
                        ...
                        <a href="{{ route('adminturnir.show',$value->id) }}">
                            <h3>{{ $value->name }}</h3>
                        </a>
                    ...
                    <div class="action-public d-flex justify-content-between mb-2">
                        @if($value->isPiblic == 1)
                            <a href="{{ route('adminturnir.hidden',$value->id) }}" class="btn btn-light">Скрить</a>
                        @else
                            <a href="{{ route('adminturnir.public',$value->id) }}" class="btn btn-primary">Опублікувати</a>
                        @endif 
                        <a href="{{ route('adminturnir.delete',$value->id) }}" class="btn btn-light">Видалити</a>
                    </div>
                ...
                <div class="card-turnir shadow p-2 bg-light rounded-lg text-dark">
                    ...
                    <div class="action-public d-flex justify-content-between mb-2">
                        ...
                        @else
                            <a href="{{ route('adminturnir.public',$value->id) }}" class="btn btn-primary">Опублікувати</a>
                        @endif 
                        <a href="{{ route('adminturnir.close',$value->id) }}" class="btn btn-success">Завершити</a>
